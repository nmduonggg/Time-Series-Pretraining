...
def load_model(args):
    ...
    master_print("[WARN] Using %s in this experiment" % device)
    ...
    if args.weight is not None:
        print('Load weight from %s' % args.weight)
        
        # remove 'module.' part
        time_enc_state_dict = torch.load(os.path.join(args.weight, 'time_encoder.pt'), map_location=torch.device(device))
        time_enc_state_dict = {
            k[7:]: v for k, v in time_enc_state_dict.items() if 'input_embedding' not in k  # avoid mismatch
        }
        ch_enc_state_dict = torch.load(os.path.join(args.weight, 'channel_encoder.pt'), map_location=torch.device(device))
        ch_enc_state_dict = {
            k[7:]: v for k, v in ch_enc_state_dict.items() if 'proj_out' not in k
        }
        
        time_enc.load_state_dict(time_enc_state_dict, strict=False)
        ch_enc.load_state_dict(ch_enc_state_dict, strict=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--start_epo_id", type=int, default=-1)
    ...
    parser.add_argument("--mask_by_channel", action="store_true")
    parser.add_argument("--rand_mask", action="store_true")
    parser.add_argument("--mask_len", type=int)
